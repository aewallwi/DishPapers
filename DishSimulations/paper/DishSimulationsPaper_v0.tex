...
\begin{figure}
\includegraphics[width=.5\textwidth]{figures/convolveCompare.pdf}
...
Rather, we employ CST's time domain mode to simulate the response of the dish to plane waves incident off of zenith to understand the frequency dependence of the side-lobes using the same technique that was described in \S~\ref{ssec:Deconvolve}. In Fig.~\ref{fig:Comparison}, we show the delay structure of the sidelobe response functions we obtain on a N-S line $20^\circ$ and $30^\circ$ from zenith, which approximately correspond to the sidelobes of the beam. 
