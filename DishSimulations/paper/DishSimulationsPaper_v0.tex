...
It is instructive to examine, in more detail, the case where $r_0$ at ($\tau=0$) is significantly greater than the other coefficients. Than we can approximate
\begin{equation}
...
If we make the assumption that the beam can be factored into a frequency dependent and frequency dependent term, $a_m = a({\bf \hat{k}}) r_m$ than we have
\begin{equation}
\widetilde{V}_{ij} \approx \widetilde{V}_{ij}(\tau) + \sum_{m>0} r_m \widetilde{V}_{ij}(\tau - \tau_m)
\end{equation}
 Hence, for a delay response dominated by the $0$-delay mode, as we might expect in our antenna element, the delay transformed visibilities are effectively convolved with the {\it voltage response}, not the voltage response squared as we might naively expect!


 In this paper, we study $R_{mn}({\bf \hat{k}})$ for the HERA antenna element to verify that it both meets the spec and agrees with direct measurements of the antenna element with refelctometry \citep{Patra:2015} and Orbcomm measurements \citep{Neben:2015b}.
