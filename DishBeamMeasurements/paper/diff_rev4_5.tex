...
The HERA element (Fig. \ref{fig:feeddiagram}) is a 14\,m diameter faceted \DIFdelbegin \DIFdel{parabolic dish }\DIFdelend \DIFaddbegin \DIFadd{paraboloid }\DIFaddend ($ f/D=0.32$) with a dual-polarized dipole feed suspended at prime focus \citep{dishmemo}. Here $f$ is the focal length of the dish and $D$ is the dish diameter. The dish surface is formed by wire mesh sheets (i.e., facets) mounted on PVC tubes which run from the lip of the dish to the hub at the vertex. For these tests, the feed consists of a dual linear polarization PAPER sleeved dipole mounted 17'' below a 78'' diameter wire mesh back plane surrounded by a 30'' deep cylinder\DIFdelbegin \DIFdel{, suspended from cables attached to each of the }\DIFdelend \DIFaddbegin \DIFadd{. The feed is suspended from a single point on its back plane from three ropes, each attached to a telephone pole. The }\DIFaddend three telephone poles \DIFaddbegin \DIFadd{are equally spaced }\DIFaddend around the dish. The dipole ``sleeves'' are circular disks just above and below the dipole designed to broaden its frequency response. The \DIFaddbegin \DIFadd{feed }\DIFaddend cylinder is offset 0.5'' from the back plane, and is designed to make the \DIFdelbegin \DIFdel{beams for both linear polarizations more similar}\DIFdelend \DIFaddbegin \DIFadd{dipole beam more azimuthally symmetric and also taper its response near the edges of the dish to reduce spillover into adjacent dishes}\DIFaddend . Fig. \ref{fig:feedphoto} shows the feed as deployed on the ground for early testing. 
\DIFaddbegin 

\DIFaddend The nominal dish focus is \DIFdelbegin \DIFdel{$f=(f/d)D=4.48$}\DIFdelend \DIFaddbegin \DIFadd{$f=(f/D)D=4.48$}\DIFaddend \,m, though given its faceted design, the dish does not have a single focus. Our numerical electromagnetic models suggest the best focus is slightly higher \DIFdelbegin \DIFdel{due to the feed geometry}\DIFdelend \DIFaddbegin \DIFadd{than that of an perfect paraboloid}\DIFaddend . In this work we study the dish beam pattern at rigging heights of 4.5\,m, 5.0\,m, and 5.3\,m, measured from dish surface to feed plane, the last height being the maximum height we can achieve with the feed suspension system installed on the dish. These height measurements are uncertain at the $\pm5\%$ level in this study. For more details on the dish design and construction see \citet{deboer16}. Feed \DIFdelbegin \DIFdel{/dish }\DIFdelend optimization studies are ongoing and the values of these parameters may change in the full HERA array \citep{feedoptimizationmemo}. 
...
\caption{Simulated dish power patterns (NS polarization) at 137\,MHz (see Sec. \ref{sec:dishmodels}) with $h_\text{feed}=5$\,m using \DIFdelbeginFL \DIFdelFL{HFSS }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the full-faceted model }\DIFaddendFL (left) and \DIFdelbeginFL \DIFdelFL{CST }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the perfect paraboloidal model }\DIFaddendFL (middle) are shown beside an ideal Airy pattern for a 14\,m diameter dish for comparison. Dased lines mark zenith angles of 20$^\circ$, 40$^\circ$, 60$^\circ$, and 80$^\circ$.}
\label{fig:modelbeams}
\end{figure*}

We numerically model the HERA dish in \DIFdelbegin \DIFdel{both ANSYS HFSS}\footnote{\DIFdel{http://www.ansys.com/Products/Electronics/ANSYS-HFSS}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{and in CST Microwave Studio}\footnote{\DIFdel{https://www.cst.com/Products/CSTMWS}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{, making slightly different assumptions, }\DIFdelend \DIFaddbegin \DIFadd{two different ways }\DIFaddend in order to study the range of realistic beams given modeling inaccuracies and material imperfections. In particular, the near horizon beam response, which sets the level of horizon brightening in the delay spectrum, is quite sensitive to modeling assumptions. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The HFSS model uses a realistic faceted dish }%DIFDELCMD < \citep{deboer16} %%%
\DIFdel{($f/D=0.32$) with a }\DIFdelend \DIFaddbegin \DIFadd{We first generate a full-faceted model of the dish using ANSYS HFSS}\footnote{\DIFadd{http://www.ansys.com/Products/Electronics/ANSYS-HFSS}}\DIFadd{. All mesh surfaces are modeled as solid aluminum and the dipole itself is modeled as copper. The }\DIFaddend 1\,\DIFdelbegin \DIFdel{m diameter dielectric similar to dry soil at the vertex. In reality a 1}%DIFDELCMD < \,%%%
\DIFdelend m \DIFdelbegin \DIFdel{diameter }\DIFdelend \DIFaddbegin \DIFadd{concrete }\DIFaddend circle at the vertex is \DIFdelbegin \DIFdel{filled with concrete and supports the PVC pipes which suspend the mesh panels, but we effectively assume here it is perfectly absorbant.  The feed is modeled as a sleeved dipole with a solid metal back plane and cylinder with the dimensions given above. Copper is used for the dipole itself, and aluminum for all reflecting surfaces. In the simulations, we excite one of the dipoles using a modal port and measure the total gain response in each direction to 137.5}%DIFDELCMD < \,%%%
\DIFdel{MHz radiation. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The CST model assumes a 14}%DIFDELCMD < \,%%%
\DIFdel{m diameter perfect paraboloidof solid aluminum with $f/D=0.32$ but neglects the 1}%DIFDELCMD < \,%%%
\DIFdel{m hole at the vertex, effectively assuming the concrete and earth ground behind it are perfectly reflective. The feed model has the same dimensions and properties as in the HFSS model. These simulations are done by exciting one of the dipoles with band-limited noise (100--200}%DIFDELCMD < \,%%%
\DIFdel{MHz), then measuring the farfield radiation in each direction after the excitation pulse energy within the structure decays to below -80}%DIFDELCMD < \,%%%
\DIFdel{dB}\DIFdelend \DIFaddbegin \DIFadd{modeled with a dielectric similar to dry soil. For comparison, we also model the dish as a perfect paraboloid}\DIFaddend . \DIFaddbegin \DIFadd{We simulate this second model using CST Microwave Studio}\footnote{\DIFadd{https://www.cst.com/Products/CSTMWS}}\DIFadd{, but the differences are dominated by the dish geometry, not the choice of numerical electromagnetic solver. 
}\DIFaddend 

The simulated \DIFdelbegin \DIFdel{HFSS and CST }\DIFdelend \DIFaddbegin \DIFadd{full-faceted and perfect paraboloid }\DIFaddend beams for the NS dipole are plotted in Fig. \ref{fig:modelbeams} (left and center panel) along with an Airy pattern for comparison. As expected, both model beams have slightly stronger sidelobes and wider main lobes than the ideal Airy pattern. The dipole sleeve (circular pieces in Fig. \ref{fig:feedphoto}) and skirt result in a feed beam which is slightly elongated in the E plane and slightly compressed in the H plane, opposite to the behavior of a simple dipole. This wider dish illumination in the NS direction by the NS feed dipole results in a narrower dish beam in the NS direction. Similarly, the EW dish beam is narrower in the EW direction. Lastly, we note that in both models, the best focus is found to be close to 5.23\,m with this feed geometry.
...
Figure \ref{fig:measuredbeamslices} shows slices through the E and H planes of these power patterns along with the \DIFdelbegin \DIFdel{HFSS and CST }\DIFdelend \DIFaddbegin \DIFadd{full-faceted and perfect paraboloid }\DIFaddend numerical models discussed earlier. As in the previous plot, the EW and NS beams are shown in the left and right panels, while the different feed heights are shown in the different rows. The data agree with both models to within 1\,dB in the main lobe, though in several cases appear slightly shifted so they are not quite centered on zenith. The data diverge further in the sidelobes at zenith angles of $20^\circ$ and larger. Here the evolution of the sidelobes as the feed is raised is again seen starkly, as is the fact that the main lobes are slightly wider along the H planes than along the E planes. We observe that both models agree with the measured beams in the main lobe but deviate from the data in different ways at the 1--5 dB level in the sidelobes. Neither model \DIFdelbegin \DIFdel{looks consistently better }\DIFdelend \DIFaddbegin \DIFadd{agrees consistently better with the data}\DIFaddend , suggesting that \DIFdelbegin \DIFdel{the as-built HERA dish deviates from both numerical models more significantly than the models deviate from each other due to their slightly different }\DIFdelend \DIFaddbegin \DIFadd{real-world imperfections of the HERA dish dominate over the slightly different modeling }\DIFaddend assumptions. 

We emphasize that the model deviations observed in the measured beams are real in that they are larger than the 0.5\,dB scale systematics observed in the null experiments (Fig. \ref{fig:nullexptplots}). Those experiments bound the impact of environmental reflections and reference dipole mismodeling to the 10\% level or smaller across the whole sky. The observed dish beam asymmetries, model deviations in sidelobes, and slight shifts of the main lobes all suggest feed centering errors. The feed is suspended by \DIFdelbegin \DIFdel{rope from }\DIFdelend \DIFaddbegin \DIFadd{three ropes attached from the center of the feed back plane to }\DIFaddend three telephone poles spaced around the dish, and is raised by pulling all three ropes to a new length. Each time this is done the feed centering is slightly disturbed because all three ropes must be pulled to the exact same length to center the feed. \DIFdelbegin \DIFdel{If feedtilt/rotation errors }\DIFdelend \DIFaddbegin \DIFadd{Because all three ropes are attached to the same point on the feed, changing their lengths does not affect feed rotation or tilt. Thus if rotation or tilt errors, }\DIFaddend or dish surface imperfections\DIFaddbegin \DIFadd{, }\DIFaddend were significant, then the beam errors at different feed heights would look similar\DIFdelbegin \DIFdel{because these errors persist when the feed is raised or lowered. All three ropes are tied to a single location on the feed, so rope length errors affect on the feed centering, not its rotation or tilt. }\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend The fact that the observed model deviations change \DIFdelbegin \DIFdel{over the three beam measurements }\DIFdelend \DIFaddbegin \DIFadd{with feed height }\DIFaddend suggests that feed centering errors are most significant\DIFdelbegin \DIFdel{because they are the only ones which change when the feed height is changed}\DIFdelend . To mitigate \DIFdelbegin \DIFdel{these }\DIFdelend \DIFaddbegin \DIFadd{all these feed positioning }\DIFaddend errors, the feeds in the full HERA array will be tied down to the dish surface at several points.
...
The collecting areas are shown in Table 1 along with the maximal collecting area achieved by the Airy pattern for a 14\,m dish. The measured collecting areas \DIFdelbegin \DIFdel{are 30--50}\DIFdelend \DIFaddbegin \DIFadd{imply aperture efficiencies of 45--60}\DIFaddend \%\DIFdelbegin \DIFdel{lower than the geometric area}\DIFdelend . This is in line with expectations given \DIFdelbegin \DIFdel{that the Airy pattern has the largest possible collecting area, equal to the dish cross section, and the feed's backplane and cylindrical skirt reduce it. However we opt for this reduction over added collecting area }\DIFdelend \DIFaddbegin \DIFadd{the feed design which tapers the dipole beam towards the edges of the dish to reduce spillover into adjacent dishes. The mesh cylinder hanging from the feed back plane around the dipole also reduces the aperture efficiency slightly }\DIFaddend in order to \DIFdelbegin \DIFdel{reduces the azimuthal beam asymmetry}\DIFdelend \DIFaddbegin \DIFadd{make the feed beam more azimuthally symmetric}\DIFaddend . 
...
\hline
  Airy pattern & 155 & \DIFdelbeginFL \DIFdelFL{25.5}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{18.7}\DIFaddendFL , 90.8  \\
    Measured, feed at 5.3\,m & 93.0 & \DIFdelbeginFL \DIFdelFL{19.3}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{12.7}\DIFaddendFL , 74.3 \\
    Measured, feed at 5\,m & 77.1 &  \DIFdelbeginFL \DIFdelFL{16.4}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{10.6}\DIFaddendFL , 67.9 \\
    Measured, feed at 4.5\,m & 68.5 &  \DIFdelbeginFL \DIFdelFL{15.4}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{10.0}\DIFaddendFL , 63.9 \\ 
  \hline
\end{tabular}
\end{table}
...
We run 21cmSense\footnote{https://github.com/jpober/21cmSense} to compute the overall SNR of a power spectrum detection with one season (6 hours per night for 180 nights) of HERA-320 data. We use a fiducial Epoch of Reionization model generated with 21cmFast \citep{21cmfast}. This model assumes $\zeta=31.5$ for the ionizing efficiency, $T_\mathrm{vir}=1.5\times10^4$\,K for the minimum virial temperature of halos producing ionizing photons, and $R_\text{mfp}=30$\,Mpc for the mean free path of ionizing photons, and reaches 50\% ionization at $z \sim 9.5$ and complete ionization at $z \sim 7$, and is consistent with \DIFdelbegin \DIFdel{most }\DIFdelend current observations \citep[e.g.][]{PoberNextGen}. 
\DIFaddbegin 

\DIFaddend We predict SNRs first for a foreground \textit{avoidance} approach where only modes outside of the wedge plus a buffer of \DIFdelbegin \DIFdel{$\Delta k_\parallel=0.1\,\mathrm{h}\,\mathrm{Mpc}^{-1}$ }\DIFdelend \DIFaddbegin \DIFadd{$\Delta k_\parallel=0.15\,\mathrm{h}\,\mathrm{Mpc}^{-1}$ }\DIFaddend are used. These modes have frequency dependence larger than that of any \DIFaddbegin \DIFadd{smooth spectrum }\DIFaddend source on the sky\DIFaddbegin \DIFadd{, and this buffer size is chosen to exclude modes which leak out of the wedge due to beam frequency dependence. Due to imperfect impedance matching at the center of the 100-200}\,\DIFadd{MHz band, the $z\sim8.5$ band requires a slightly larger buffer, though our chosen buffer effectively avoids the leakage in other bands }\citep{ewallwice16}\DIFaddend . We also predict SNRs for a foreground \textit{subtraction} approach using all modes whose instrumental frequency dependence is larger than that of a source at the edge of the main lobe. 

The SNRs computed with the measured collecting areas are \DIFdelbegin \DIFdel{15-20 }\DIFdelend \DIFaddbegin \DIFadd{10-13 }\DIFaddend with foreground avoidance compared with \DIFdelbegin \DIFdel{25 }\DIFdelend \DIFaddbegin \DIFadd{19 }\DIFaddend for the Airy pattern. With foreground subtraction, the SNR falls from 90 with the Airy pattern to 60-75 with the measured collecting areas. In all cases this reduction is a loss of sensitivity, but a power spectrum detection is still always very significant at the \DIFdelbegin \DIFdel{15}\DIFdelend \DIFaddbegin \DIFadd{10}\DIFaddend $\sigma$ level or better.
...
These deviations away from models and away from 180$^\circ$ azimuthal symmetry are larger than the $\pm0.5$\,dB systematics observed in our null experiments which probe the accuracy of our beam measurement system, suggesting they are genuine measurements of the in situ dish beam. The most likely culprit is feed mis-centering which shifts and distorts the main lobe sidelobes. In the full HERA array, the suspended feeds will be tied to the dish surface at several points to \DIFdelbegin \DIFdel{level and center them}\DIFdelend \DIFaddbegin \DIFadd{fine tune the feed centering and leveling, and mitigate wind buffeting}\DIFaddend . Characterizing the level of antenna-to-antenna beam variation in the full HERA array and its effects on power spectrum analyses, as \DIFdelbegin \DIFdel{Neben et al. (in press) }\DIFdelend \DIFaddbegin \citet{neben16} \DIFaddend do for the MWA, is left as future work.

We quantify HERA's sensitivity to the 21\,cm power spectrum given our beam measurements by first computing the collecting area of the measured beams at the different feed rigging heights, finding 93\,m$^2$ at the best focus\DIFdelbegin \DIFdel{. We observe a reduction of 30--50}\DIFdelend \DIFaddbegin \DIFadd{, implying an aperture efficiency of 60}\DIFaddend \%\DIFdelbegin \DIFdel{from the geometric cross section of a 14}%DIFDELCMD < \,%%%
\DIFdel{m dish due to feed blockage and under-illumination }\DIFdelend \DIFaddbegin \DIFadd{. Feed optimization is ongoing, but the present feed sacrifices aperture efficiency in order to taper the dipole beam towards the edges }\DIFaddend of the dish \DIFaddbegin \DIFadd{and make the X and Y dipole beams as similar as possible using a cylinder hanging from the back plane}\DIFaddend . We convert our measured collecting areas into effective dish sizes, then use 21cmSense to predict the overall power spectrum SNR at $z\sim9.5$ with one season of HERA-320 data. We predict SNRs of \DIFdelbegin \DIFdel{19.3 }\DIFdelend \DIFaddbegin \DIFadd{12.7 }\DIFaddend and 74.3 using foreground avoidance and subtraction approaches respectively, compared with SNRs of \DIFdelbegin \DIFdel{25.5 }\DIFdelend \DIFaddbegin \DIFadd{18.7 }\DIFaddend and 90.8 using an ideal unobstructed 14\,m aperture (Airy pattern). Still, these sensitivities permit a very significant detection of the 21\,cm signal after a single observing season.
...
We predict the magnitude of this effect for the HERA element \DIFdelbegin \DIFdel{as well as its dependence on beam modeling uncertainties}\DIFdelend \DIFaddbegin \DIFadd{and discuss the uncertainties in this estimate due to beam modeling uncertainty}\DIFaddend . As expected, we find that the level of horizon brightening is largest when the galaxy is just above the horizon, and lowest when it is well below. When this pitchfork effect is large, we find that the uncertainty in its predicted amplitude is also large, as seen in the differences between the delay spectra calculated using \DIFdelbegin \DIFdel{HFSS and CST }\DIFdelend \DIFaddbegin \DIFadd{full-faceted and perfect paraboloid }\DIFaddend beam models. When the effect is small, the two beam models produce much more similar results, highlighting the delay spectrum as an exquisite probe of the difficult-to-measure beam response at very low elevations. \DIFaddbegin \DIFadd{Of course the delay spectrum provides only an integrated measure of the beam, but some information can still be extracted. By forward modeling foreground delay spectra using different MWA primary beam models, for instance, it was observed that the MWA bowtie dipoles are better modeled as isotropic radiators than hertzian dipoles at these low elevations (N. Thyagarajan, private communication). Direct measurements using transmitter-equipped drones would be ideal and their development is ongoing }\citep{drone1,drone2}\DIFadd{.
}\DIFaddend 

As discussed by the other papers in this series \citep{ewallwice16,patra16,nithya16}, the frequency dependence of both the beam's angular response and its overall gain widen the delay kernel of a source, leaking power into the EOR window out to \DIFdelbegin \DIFdel{$k_\parallel=$}\DIFdelend \DIFaddbegin \DIFadd{$k_\parallel\approx$}\DIFaddend 0.15\DIFdelbegin \DIFdel{-0.23}\DIFdelend h\,Mpc$^{-1}$ \DIFdelbegin \DIFdel{. This is well inside the wedge plus }\DIFdelend \DIFaddbegin \DIFadd{over much of the 100-200MHz band. This leakage falls within the wedge }\DIFaddend buffer used in a \DIFaddbegin \DIFadd{fiducial }\DIFaddend foreground avoidance analysis\DIFdelbegin \DIFdel{. Foreground subtraction, however, requires calibrating out the instrument frequency response well enough to begin to access cosmological modes within the wedge. This is much more challenging, but the sensitivity gains are enormous as those signal modes are thought to be the strongest. Even with only foreground avoidance, though, assuming only previously demonstrated analysis }\DIFdelend \DIFaddbegin \DIFadd{, so our SNR projections take into account the sensitivity reduction due to beam chromaticity. These sensitivities can be improved using new techniques such as foreground covariance downweighting and fringe rate filtering }\citep{ali2015,parsonsoptimalfringeratefiltering}\DIFadd{, which mitigate foreground leakage into the EOR window, thereby permitting a smaller buffer. Using only these previously demonstrated }\DIFaddend techniques, we project a \DIFdelbegin \DIFdel{$19.3\sigma$ }\DIFdelend \DIFaddbegin \DIFadd{$13\sigma$ }\DIFaddend detection of the EOR power spectrum with a single observing season which would provide begin to probe reionization models in detail and shed light on our cosmic dawn. 
