...
\shorttitle{Effects of Beam Chromaticity on EoR Power Spectra Measurements}
...
Unaccounted systematics from foregrounds and instruments prevent present day experiments from detecting redshifted 21~cm signal from the Epoch of Reionization (EoR). In-depth understanding of these effects is critical for the success of such experiments. We identify spectral structures (chromaticity) in the antenna power pattern as one such systematic that significantly affects all EoR measurements. Increase in chromaticity of the antenna beam causes the already bright foreground power to leak well beyond ideal limits and contaminate the otherwise clean EoR signal modes. A primary source of this chromaticity is reflections in the antenna-feed assembly and between structures in neighboring antennas. Using precise foreground simulations taking wide-field effects into account, we provide a framework to set cosmologically-motivated upper limits on these reflections to prevent further EoR signal degradation. We use the Hydrogen Epoch of Reionization Array (HERA) as an example. Using a simple and yet high dynamic range spectral weighting scheme, we demonstrate that EoR detection using HERA will not be impeded by such spectral systematics even in a conservative scenario that does not require removal of foregrounds. We also show that all baselines in a 19-element HERA layout are capable of detecting EoR over a substantial observing window on the sky.
...
We note that these visibilities and delay power spectra are dependent on right ascension (RA) of the pointing as well as the frequency band. We defer expressing visibilities and delay power spectra explicitly as a function of pointing or center frequency (or redshift) to later sections where it becomes necessary.
...
\end{align}
In its Fourier domain, this window has a response obtained by squaring the {\it Blackman-Harris} window response and thus increases the dynamic range further in the power response by another 100 -- 120~dB while $\epsilon\approx 42\%$. This is an enormous gain in sidelobe characteristics for a negligible loss in over sensitivity relative to a simple {\it Blackman-Harris} weighting. This will ensure that the contamination resulting from spillover of foregrounds along the line-of-sight $k$-modes are limited by intrinsic spectral structures in the foregrounds and the beam patterns and not by sidelobes from spectral weighting. In this paper, we apply this modified spectral weighting function defined in equation~\ref{eqn:bhw2} whenever foregrounds are represented in the Fourier delay domain. 
