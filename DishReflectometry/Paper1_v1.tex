...
\end{equation}
After rearranging and plugging in for $p_n(\hat{\theta})$ and $\eta_f$ we find
\begin{equation}
a = p_n(\hat{\theta})\left[\eta_f + \frac{\Gamma_f}{\eta_f}\left(s_{11} - \Gamma_f\right)\right]
\end{equation}
...
\end{equation}
where $P_n(\hat{\theta})= |p_n(\hat{\theta})|^2$ is the normalized power pattern, and $\epsilon_f=|\eta_f|^2$ is the feed efficiency.
