...
% HERA feed design is derived from modified PAPER antenna and therefore the return loss closely relates to the PAPER antenna response with slight frequency shift. The return loss measured when the HERA feed is suspended over the dish is, however, show deviation from the feed performance. }
\label{fig:all_RL}
\end{figure}
\begin{figure}       
\centering
...
\label{fig:window}
\end{figure} 
%\subsubsection{\textbf{Delay spectrum of subbands}} 
%The red shifted 21cm power spectrum measurements could be divided into sub-bands around multiple frequencies corresponding to various redshifts. Assuming that the power spectrum evolved over the redshift range $\Delta z \>0.5$ (Zaldarriaga 2004, ) corresponding to a $5~MHz$ band width at $150~MHz$. 
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{plots/VNA_RL.png}
\caption{Magnitude and phase of the combined effects of the return loss at the VNA input alone with the cable resistive loss as computed from the return loss measurement of the open load at the feed input of the cable. This is consistent with the specifications of the RF connectors which is commonly used at low frequencies }
\label{fig:window}
\end{figure} 
