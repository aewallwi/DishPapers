...
The reflected signal illuminates the dish and most of it reflects back into free space.
However, a part of it is reflected back and forth several times in between the feed and the vertex of the dish, which is shadowed by the feed. %No longer an included figure (dashed blue arrows in Figure \ref{fig:cartoon}).
Such reflections generate multiple copies of the incident sky signal of reduced strength at various delays and phases and, thus, produce spurious correlations in the visibilities of interferometric data.  Identical spurious visibilities could result from reflections internal to the system causing delay space distortion. Therefore, it is necessary to measure and understand the instrument delay spectrum in order to qualify the system for the measurement of the 21cm power spectrum. \\
\indent In this section, we compute the visibility of a two element interferometer accounting for the additional reflections of the sky signal in between the feed and the dish vertex and the corresponding affects on the delay spectrum, using the multiple reflection formulation of Sec. \ref{sec:multiple}. We also examine, in detail, the affect of reflections on the visibility data and the corresponding affect on the delay spectrum. We assume two antennas with identical geometry and electrical parameters, i.e, $\bmvolt_{1}(\thhat,\nu)=\bmvolt_{2}(\thhat,\nu) = \bmvolt(\thhat,\nu)$. 
...
& = & \frac{ (1+\Gamma_d)(1+\Gamma_{f})}{1+\Gamma_{f}\Gamma_{d}\dfngexp} 
 \label{eq:Sigma}
\end{eqnarray}
Note that $n=0$ is when the initial wave enters the cable at the feed (where we assume we have set our reference plane).  We may identify $(1+\Gamma_d)(1+\Gamma_{a})$ \textbf{Shouldn't have $\mathbf{(1+\Gamma_a)}$?} as the voltage reception pattern of the antenna, denoted $\bmvolt(\thhat,\nu)$, which is a function of angle on the sky.  Additionally, $(1+\Gamma_f$) is the voltage efficiency for the feed (not a function of angle).
...
% Combining these terms, the expression of visibility will be,
...
In a related series of papers, Thyagarajan et al. (2016) have shown the simulated sensitivity calculations of HERA for various antenna element responses (Figure \ref{fig:sim_limit_delay}) using a comprehensive foreground model as well as a model for the EoR power spectrum. From these simulations, Figure \ref{fig:sim_limit_delay} (Thyagarajan et al.) shows the level of attenuation required for the multiply reflected sky signal in order to reduce the foreground contaminations at the delay modes significant for power spectrum measurements. Ewall-Wice et al. (2016) has simulated the time domain response of the HERA element considering multiple reflections between the feed and the dish.  Figure \ref{fig:sim_limit_delay} shows the simulated cross-power response of a pair of HERA antennas as a function of time (Ewall-wice et al.). These simulations set up the performance criterion for the HERA antenna element.
 \begin{figure*}[ht]
\begin{minipage}[b]{0.5\linewidth}
\centering
\includegraphics[angle=0, width=\linewidth]{plots/sim_limit.jpg}
\end{minipage}
\hspace{0.1cm}
\begin{minipage}[b]{0.5\linewidth}
\centering
\includegraphics[angle=0, width=\linewidth]{plots/sim_delay.jpg}
\end{minipage}
\caption{Left: Signal attenuation required for the reflected sky power arriving at the feed at higher delays, as estimated from the simulated antenna beam and foreground models (Thyagarajan et al.). The three different contours show three different system performance requirements based on the simulated response of the HERA element for a measurement frequency of 150MHz, corresponding to the redshift of $z=8.47$, and a 14.5m baseline. The instrumental requirements become extremely stringent for the detection of the smaller $k_\parallel$ values, corresponding to very large spatial scales. Right: The simulated cross power response kernel for HERA. The simulation assumes an achromatic beam with the chromaticity attributed entirely to the multiple reflections between the dish and the feed. This shows the level to which the multiple reflections at the higher delays is expected to be reduced following the dish and feed designs.} %Some explanation of the left hand axis of the right panel figure would be really nice in here since this is the only mention of R(tau), and it's new notation compared to the rest of the paper.
\label{fig:sim_limit_delay}       
\end{figure*}
...
%, if the incident power from the sky signal is $I_{sky}$, the feed
...
\end{minipage}
\caption{Left: Magnitude of the return loss of power measured between 100 to 200 MHz using the vector network analyzer. Right: The corresponding phase. The return loss of the feed alone is shown in green while the same for the feed+dish assembly is shown in blue. Finally, the quantity of interest ${I_{rec}\over I_{sky}}$ is shown in red. Ideally, 100\% of the sky power is received, and ${I_{rec}\over I_{sky}} = 1$, or 0dB. A finite return loss at the antenna input causes deviations in this quantity from its ideal value, and ${I_{rec}\over I_{sky}} < 1$. Also, the frequency dependence of the feed and dish, as well as the multiple reflections, introduces the small scale fluctuations shown in the red curve. }
% The HERA feed design is derived from a modified PAPER antenna, and, therefore, the return loss closely relates to the PAPER antenna response with a slight frequency shift. The return loss measured when the HERA feed is suspended over the dish, however, shows deviations from the feed performance. }
\label{fig:freq}       
\end{figure*}
...
\subsection{Feed effect: zero delay correction}
...
The return loss $\Gamma_{f}$ determines the amount of power coupled to the system via the feed. Additionally, with each reflection, the power coupled to the system reduces by a factor of $\Gamma_{f}^{2}$. Since the reflected signal results in a system response at a higher delay, multiple reflections of the signal off the feed and the dish would result in an exponential envelope in the delay spectrum. The return loss of power results from the antenna impedance mismatch with free space as well as the transmission line. The complex antenna impedance is shown in Figure \ref{fig:feed_impedance}.
\begin{figure}
\centering
\includegraphics[width=\linewidth]{plots/Feed_impedance.png}
\caption{Complex antenna impedance of the HERA feed element.} %Some indication of which line is which on the plot would be really helpful here.
\label{fig:feed_impedance}
\end{figure} 
%
While the antenna beam pattern, $A_{f}$, is a smooth function of frequency and should have its imprint confined to lower delays, the return loss, $\Gamma_{f}$, broadens the delay spectrum. However, a smaller return loss will result in a vertical shift of the delay spectrum at lower values and, therefore, can make the higher order reflections insignificant. Given the antenna impedance and the 50 Ohm transmission line, the measurements are done using a passive balun with a 2:1 impedance ratio, providing a net return loss of power between the sky, the antenna, and the transmission line shown in Figure \ref{fig:freq}. This provides the maximum impedance matching. \\
\subsubsection{Effect of frequency bandwidth on delay spectrum estimation}
The estimation of the delay spectrum by taking the Fourier transform of the measured return loss is sensitive to the frequency bandwidth of operation. Since the measurements are done in the spectral domain, a measurement between two frequencies is analogous to windowing the frequency domain data by a square window function. In the delay domain, the Fourier transform of this would result in multiple side lobes at higher delays. Therefore, appropriate windowing of the data is necessary prior to taking the Fourier transform of the measured data. Figure \ref{fig:window} shows the delay spectrum estimated from the measured data using a square window as well as a Blackman Harris window. Windowing prior to Fourier transforming the data significantly reduces the system response at higher delays.  
\begin{figure}
\centering
\includegraphics[width=\linewidth]{plots/window.png}
\caption{The affect of a finite bandwidth on the estimation of the delay spectrum: The blue line shows the delay spectrum of the HERA element computed from the measured data, which is band limited between 100 to 200MHz. The green line shows the delay spectrum estimated from the same data set after multiplying the data by a Blackman Harris window. The delay spectrum of the windowed data set shows a significant reduction in the instrument response at higher delays.}
\label{fig:window}
\end{figure} 
\subsubsection{Effect of the dish and feed on the delay spectrum estimation}
The measurement equations show the effects of multiple reflections of the sky signal between the feed and the dish. Any other reflections off the feed structure and/or any other part of the dish-feed assembly would appear at arbitrary delays. The estimated delay spectrum of the instrument is collectively contributed to by all the reflections. We measure the return loss of the feed alone, keeping it on the ground facing the sky (Figure \ref{fig:freq}). The delay spectrum estimated from this measurement provides the lowest limit of the delay spectrum of the instrument, considering there are not multiple reflections between the feed and the dish. 
 Any additional reflections also contribute to the measured return loss and, thus, the delay spectrum at various delays. Figure \ref{fig:delay_spectrum} shows the delay spectrum of the feed while it is kept upside down on the ground (in green), as well as that of a HERA antenna with the feed suspended at the focal plane (in blue). The red line shows the delay spectrum of the PAPER antenna for comparison with the HERA element.
%\subsubsection{Effect of feed height on delay spectrum estimation}
\subsubsection{Performance Comparison: HERA vs. PAPER}
While PAPER has established the, to date, most stringent upper limit on the 21cm power spectrum measurement, the need to increase the measurement sensitivity called for increased collecting area per PAPER antenna. This has driven the parabolic reflector antenna design of the PAPER successor, HERA, where essentially the PAPER antenna, with minor modifications of the ground plane-side plane assembly, works as the feed and, thus, in combination with the dish, yields a larger collecting area per element. Figure \ref{fig:freq} shows the return loss measurement for the PAPER antenna, and Figure \ref{fig:delay_spectrum} shows the corresponding delay spectrum (red line). The HERA feed element shows an improved delay response compared to the PAPER element across all delays $>50~ns$, with marginal improvement at delays $<50~ns$. The delay response of the HERA element degrades marginally as the feed is suspended over the dish due to multiple reflections between the feed and the dish. The majority of structure in the response at lower delays is due to the feed and its supporting structure, evidenced by those features common to all three measurements. At the lower delays, the structures in the delay response remain unaltered even with the variation of the feed height relative to the dish vertex. Essentially these structures set lower limits on the $k_{||}$ values that could be probed, and since redshift $z$ maps to frequency (and, thus, $k_{||}$), any redshift above the instrument footprint could likewise be probed. %Check for accuracy....
