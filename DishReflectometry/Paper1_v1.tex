...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{microsys.pdf}
\caption{System}
\label{fig:sys}
\end{figure}

\noindent
Defining $\Sigma_{\nu}=\volt_{rec}/\volt_{sky}$ and we see that 
 \begin{equation}
\Sigma_{\nu}  =   (1+\Gamma_d)(1+\Gamma_{f}) \displaystyle\sum\limits_{n=0}^{N} [\Gamma_{f}\Gamma_{d}\dfngexp]^{n}
   \label{eq:Sigma}
\end{equation}
Note that $n=0$ is when the initial wave enters the cable at the feed (where we assume we have set our reference plane).  We may identify $(1+\Gamma_d)$ as the voltage reception pattern of the antenna, denoted $p_n(\theta,\phi)$, which is a function of angle on the sky.  Additionally, $(1+\Gamma_f$) is the voltage efficiency for the feed (not a function of angle), denoted $\eta_f$.

(As an aside, note that the sum in Eq. \ref{eq:Sigma} equals:
\begin{equation}
\Sigma_{\nu} =  (1+\Gamma_d)(1+\Gamma_{f})\frac{1+\Gamma_{f}^{N+1}\Gamma_{d}^{N+1}e^{(N+1)i\theta}}{1+\Gamma_{f}\Gamma_{d}e^{i\theta}} \nonumber
\end{equation}
which, in the limit becomes:
\begin{equation}
\Sigma_{\nu} = \frac{ (1+\Gamma_d)(1+\Gamma_{f})}{1+\Gamma_{f}\Gamma_{d}e^{i\theta}} \nonumber
\end{equation} 
...
\begin{equation}
\Sigma_{\nu} = (1+\Gamma_d)\frac{\Gamma_f}{(1+\Gamma_{f})}\left(s_{11} + \frac{(1+\Gamma_f)^2}{\Gamma_f}- \Gamma_f\right)
\end{equation}
After rearranging and plugging in for $p_n(\theta,\phi)$ and $\eta_f$ we find
\begin{equation}
\Sigma_{\nu} = p_n(\theta,\phi)\left[\frac{\Gamma_f}{\eta_f}\left(s_{11} - \Gamma_f\right) + \eta_f\right]
\end{equation}
By measuring the feed separately, we may use that result for $\Gamma_f = s_{11,f}$ and $\eta_f = 1 + s_{11,f}$.

For power, we want the squared magnitude, or:
\begin{equation}
\raggedright
|\Sigma_{\nu}|^2 =  P_n(\theta,\phi)\times ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\nonumber
\end{equation}
\begin{equation}
             \left[\epsilon_f +  2\operatorname{Re}\left(\frac{\Gamma_f}{\eta_f}(s_{11} - \Gamma_f)\right) 
                              + \frac{|\Gamma_f|^2}{\epsilon_{f}}|s_{11} - \Gamma_f|^2 \right]
\end{equation}
where $P_n(\theta,\phi)= |p_n(\theta,\phi)|^2$ is the normalized power pattern, and $\epsilon_f=|\eta_f|^2$ is the feed efficiency.
