...
another. In addition to that, systematic effects are unique to each instruments and their manifestation 
...
In this paper, we investigate the performance of an individual HERA element and resulting contribution of the instrument response to the delay spectrum.
...
Consider a two element interferometer with a baseline $\vec b$. If we denote the electric field sky signal in direction $\thhat$ by $\volt_\sky(\vec \theta, \nu)$  then the voltage output of each antenna may be written as,  
\begin{equation}
\volt_{1}(\thhat, \nu) = \bmvolt_{1}(\thhat,\nu)\volt_\sky(\thhat,\nu)\nonumber\\
\end{equation}
\begin{equation}
...
where
\begin{equation}
  k_\perp = \frac{2\pi f}{D}({b\over c}), \\
...
$z$: cosmological redshift corresponding to frequency of observation.\\
...
In this paper, we use $\Omega_{M}=0.27$, $\Omega_{\Lambda}=0.73$, $\Omega_{K}=1-\Omega_{M}-\Omega_{\Lambda}$, $H_0=100\,$km$\,$s$^{-1}\,$Mpc$^{-1}$, and $P_\textrm{d}(k_\perp,k_\parallel)$ is in units of K$^2 (h^{-1}$~Mpc$)^3$.


Since both $k_{\perp}$ and $k_{\parallel}$ are functions of the delay $\tau$ corresponding to the given baseline length, the redshifted 21 cm power spectrum could be estimated from the measurement of delay spectrum of visibility by an interferometer. Sky delay spectrum from any direction $\thhat$ is convolved with the delay spectrum of the instrument and would be located at the delay $\tau = {\vec b \cdot \thhat \over c}$ %defined this differently earlier
 ...
 Hence, the sky contribution from any direction would be confined within $-\tau_{g}<\tau<\tau_{g}$. Due to chromaticity of the sky signal as well as instrument reponse, the delay spectrum of the sky spills over the delay $\tau> \tau_{g}$ with decaying amplitude [Ref to the figure Parsons12]. Sky delay spectrum $P_{sky}(\tau)$ is contributed by the foreground  $P_{fg}(\tau)$  which is spectrally smooth and the 21~cm power spectrum $P_{21}(\tau)$ which contains spectral signatures.Therefore, in the spill over region $(\tau>\tau_{g})$, the relative strength of the smooth spectrum foreground with respect to the delay spectrum of the $I_{21}$ reduces and the 21~cm delay spectrum could be detectable.
...
& = & \frac{ (1+\Gamma_d)(1+\Gamma_{f})}{1+\Gamma_{f}\Gamma_{d}\dfngexp} 
 \label{eq:Sigma}
\end{eqnarray}
Note that $n=0$ is when the initial wave enters the cable at the feed (where we assume we have set our reference plane).  We may identify $(1+\Gamma_d)(1+\Gamma_{a})$ as the voltage reception pattern of the antenna, denoted $\bmvolt(\thhat,\nu)$, which is a function of angle on the sky.  Additionally, $(1+\Gamma_f$) is the voltage efficiency for the feed (not a function of angle).
...
Accurate measurement of this quantity requires receiving the $v_{sky}$ from a well calibrated wideband source in the sky in the far field of the HERA antenna element with significant isotropic emission. While this condition is hard to achieve in practice, due to reciprocity of the antenna performance in the transmission and reception mode, the right hand side of the equation \ref{eq:Sigma} could be measured using the returnloss measurement technique with a vector network analyser. 
A vector network analyzer (VNA) is connected to the HERA element via a 50 feet long cable. The VNA transmit a broadband noise voltage  $\volt_{trans}$. $(1+\Gamma_f)$ fraction of this voltage is radiated by the feed while the fraction $\Gamma_{f}$ returns to the VNA. While the radiated signal illuminated the dish and most of it is radiated into the space,  $\Gamma_d$ fraction of this signal is reflected back towards the feed and $(1+\Gamma_f)$ is received by the VNA.  The received voltage after $n$ reflections between the dish and the feed, (where again $n=0$ is the first reflected signal at the reference plane) is therefore:
...
The fundamental difference between the righ hand side of the equation \ref{eq:Sigma} and \ref{eq:s11} are the $n=0$ term and the multiplicative factor in front of the convergent series $\displaystyle\sum\limits_{n=1}^{N} [\Gamma_{f}\Gamma_{d}\dfngexp]^{n}$ that represents the contribution of the delayed voltage components at the VNA input arising due to multiple signal reflection between the feed and the dish. Finally, writing $\volt_{rec}/\volt_{trans}=s_{11}$, equation \ref{eq:s11} could be written as,
\begin{equation}
...
\end{eqnarray}
When the feed return loss is measured separately, all the higher order terms of the equation \ref{eq:s11} does not exist. In this case $\Gamma_f = s_{11}^{f}$. Using this measurements and VNA measurement of $s11$, the quantity $v_{rec}\over v_{sky}$ is estimated.

From this, for a two element interferometer with identical antenna elements, the ratio of the received sky intensity to true sky intensity will be, 
\begin{eqnarray}
{I_{rec}\over I_{sky} } & = & |{\volt_{rec}\over \volt_{sky} }|^2 =  \beam(\thhat,\nu)\times \nonumber\\
...
Comparing eq \ref{eq:power_ratio} with \ref{eq2}, the cross power generated by $v_{1}$ and $v_{2}$ would have spurious visibility response due to mutual correlation between multiply reflected voltages represented by the second and the third term of the right hand side of the equation \ref{eq:power_ratio}. In ideal case, $\Gamma_{f}=0$ in which case equation \ref{eq:power_ratio} results in equation \ref{eq2}.  Fourier transform of this visibility spectrum along the frequency axis results in the delay spectrum. 
In the delay domain, visibility contributed by any two voltage components from two antennas with no mutual delay will be located at the delay $\tau = 0$ whereas any two voltage components from two antennas components having a mutual delay of $n\Delta \tau$ will be centred at $n\Delta \tau$. These will broaden the width of the delay spectrum which finally interferes with the 21cm power spectrum detection.
...
In a related series of papers, (Thyagarajan et al 2016) has shown the detailed sensitivity calculations of HERA is using a comprehensive foreground as well as model for EoR power spectrum for various simulated antenna element response (\ref{fig:sim_limit_delay}) From this simulations, the figure \ref{fig:sim_limit_delay} (Thyagarajan et al) shows the level of attenuation required for the multiple reflected sky signal in order to reduce the foreground contaminations at the delay modes significant for power spectrum measurements. Ewall-Wice et al 2016 has simulated the time domain response of the HERA element considering multiple reflections between the feed and the dish.  Figure \ref{fig:sim_limit_delay} shows the simulated cross-power response of a pair of HERA antenna as a function of time(Ewall-wice et al). These simulations set up the performance criterion for the HERA antenna element.
 \begin{figure*}[ht]
...
\end{minipage}
\caption{Left: Signal attenuation required for the reflected sky power arriving at the feed at higher delay as estimated from the simulated antenna beam and froeground models (Thyagarajan et al). The three different contour shows three different system performance requirements set based on the simulated response of HERA element for a measurement frequency of 150MHz corresponding to the redshift of $z=8.47$ and a 14.5m baseline. The instrumental requirements become extremely stringent for the detection of the smaller $k_\parallel$ values corresponding to very large spatial scales. Right: The simulated cross power response kernel for HERA. The simulation assumes and achromatic beam with the chromaticity attributed entirely by the multiple reflections between the dish and the feed. This shows the levels to which the multiple reflections at the higher delays is expected to be reduced following the dish and the feed design.}
...
Section 3 laid the mathematical framework of how the delay spectrum is estimated from the return loss measurement. 
...
$200MHz$ when the feed is suspended at the focal plane of the parabolic dish which is at $5m$ from the dish vertex. This measurement is Fourier transformed to obtain the delay spectrum of the instrument in the receiving mode as given in equation \ref{eq3}. Measurement bandwidth is kept much larger than the bandwidth of operation to achieve higher delay resolution. A number of systematic effects as well as mathematical artefacts that are critical to this measurements are taken in to account in determination of the delay spectrum and described below. 
...
While measuring the return loss of the dish-feed assembly, the signal transmitted by the VNA is reflected first off the feed. This essentially represents a mismatch in between the feed and the 50 Ohm transmission line. In delay domain, this appears at the zero delay bin. The for the feed, the zero delay response is identical whether or not the feed is suspended on the dish and  Therefore, we measure this response by measuring the feed return loss alone while the feed is kept on the ground facing the sky. This results in feed response alone including the feed cage structure but excluding the dish response. The using the return loss measurement of the feed $(\Gamma_{f})$, the return loss measurement $s_{11}$ of the dish and feed assembly is corrected via equation \ref{eq:power_ratio} and the quantity of interest ${I_{rec}\overI_{sky}}$ is estimated. 
