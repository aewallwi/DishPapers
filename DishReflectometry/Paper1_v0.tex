...
Consider a two element interferometer with a baseline $b$ and antenna field pattern $A(\theta, \nu)$. If we denote the sky signal at an angle $\vec \theta$ by $V_{sky}(\theta)$ and the receiver noise of each element as $V_{rec}$ then the voltage output of each antenna may be written as,  
\begin{equation}
V_{1}(\vec \theta, \nu) = V_{sky}(\vec \theta,\nu)+V_{rec1}(\nu)
\end{equation}
\begin{equation}
V_{2} (\vec \theta, \nu)= V_{sky}(\theta,\nu)exp({2\pi i \nu \vec{b}.\hat{s} \over c} )+V_{rec2}(\nu)
\end{equation}
where $\hat s$ is the unit vector along the direction of the baseline.
Hence, the time averaged complex visibility integrated over all sky as measured by the interferometer may be written as, 

\begin{equation}
V e^{i\phi} = \left< \int A_{1}(\theta, \nu) V_{1}(\theta,\nu) A_{2}^{*}(\theta, \nu) V_{2}^{*} (\theta, \nu) exp({2\pi i \nu \vec{b}.\hat{s} \over c} ) d\Omega \right >
\end{equation}



Defining antenna power pattern as  $B(\theta,\nu)=A_{1}(\theta,\nu)A_{2}^{*}(\theta,\nu)$ and denote the sky power as  $P_{sky}=V_{sky}(\theta,\nu)V_{sky}^{*}(\theta,\nu) = |V_{sky}|^2$. Since $\left < V_{rec1}(\nu)V_{rec2}^{*}(\nu) = 0\right >$
Hence, 
\begin{eqnarray}
V e^{i\phi} & = & \int B(\vec \theta, \nu) P_{sky}(\vec \theta,\nu) exp({2\pi i \nu \vec{b}.\hat{s} \over c} ) d\Omega \nonumber\\
		& = & \int B(l,m, \nu) P_{sky}(l,m,\nu) exp(2\pi i \nu \tau_{g} ) dl dm \nonumber\\
 		& = & B(\vec u,\nu) \ast P_{sky}(\vec u,\nu ) \nonumber\\
		& = & B(\nu { |\vec b| \over c} , \nu) \ast P_{sky}(\nu { |\vec b| \over c} , \nu) \nonumber\\
		& = & B(\tau_{g}, \nu) \ast P_{sky}(\tau_{g} , \nu)	
...
\bibitem[Pasrons \& Backer(2009)]{ParsonsBacker2009}
