...
\begin{abstract}
Spectral structure of the systematics has so far been the most dominant
limiting factor for the experiments attempting to detect the redshifted 21~cm
signal from the Epoch of Reionization (EoR). Systematics are specific to
instruments and their direct measurement or estimation is not always
straightforward. The Hydrogen Epoch of Reionization Array (HERA) uses the delay
spectrum technique for estimating of 21~cm power spectrum. In a unique way, the
...
effects in individual array element.  This paper presents a detailed
investigation of the instrument chromaticity of an antenna element that
constitute Hydrogen Epoch of Reionization Array. Reflectometry measurements are
carried out on a HERA dish-feed assembly between 100 to 200~MHz which studies
the multiple reflections in the system that finally results in various spurious
spectral response in the measured visibility. Reflectometry measurements shows
that even without incorporation of any sophisticated analysis method, the
instrument is capable of measuring the line of sight component  of the $k$ mode
$k_{\parallel} >0.2 h Mpc^{-1}$ modes of power spectrum with very low
foreground contaminations around $k_{\parallel} \approx0.2 h Mpc^{-1}$.  The
performance is significantly better at the high frequency subbands of
observation. When combined with a powerful foreground suppression method such
as inverse covariance weighting, the instrument is capable of measuring the
power spectrum for the spatial modes as low as $k_{\parallel} >0.1$. The study
presented here qualifies HERA as a power spectrum instrument and serves as a
fundamental method to design and measure instrument systematics for any future
experiment that aims to detect the 21~cm EoR power spectrum.  
\end{abstract}
