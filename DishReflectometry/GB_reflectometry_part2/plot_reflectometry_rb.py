...
for filename in sys.argv[1:]:
    ...
    fq_f,db_f = fromcsv('measurement_FeedOnly_DB.csv')#Reading Rich's feed only file to do the zero delay corrections.
    fq_f,ph_f = fromcsv('measurement_FeedOnly_P.csv')
    d_f = 10**(db_f/20) * np.exp(2j*np.pi*ph_f/360) # Converting the data to linear date. 20 to put into voltage amplitude, not power

    
    valid = np.where(np.logical_and(fq < .200, fq > .100)) # restrict to PAPER band
    
    fq, fq_f, d, db, ph, d_f, db_f, ph_f = fq[valid], fq_f[valid],d[valid], db[valid], ph[valid], d_f[valid], db_f[valid],ph_f[valid]
    ...
    if True: # approx account for 1st reflection of sky signal off of feed
        ...
        #_dw1 = np.fft.ifft((np.abs(d))**2*window) /window.mean()# compensate for window amplitude
        #_dw2 = np.fft.ifft((np.abs(d_f))**2*window) /window.mean()# compensate for window amplitude
        #_dw3 = np.fft.ifft((np.abs(d1))**2*window) /window.mean()# compensate for window amplitude
        #_dw4 = np.fft.ifft((np.abs(d2))**2*window) /window.mean()# compensate for window amplitude
        #_dw5 = np.fft.ifft((np.abs(d3))**2*window) /window.mean()# compensate for window amplitude
        #_dw6 = np.fft.ifft((np.abs(d_t))**2*window) /window.mean()# compensate for window amplitude
    ...
    plt.plot(fq,10.0*np.log10(np.abs((d_f)**2)),linewidth=2.5,label='Feed Abs amp')
    ...
    #plt.plot(np.fft.fftshift(tau), 10.0*np.log10(np.fft.fftshift(np.abs(_dw1)**1)), linewidth=2.5, label='Dish+Feed')
