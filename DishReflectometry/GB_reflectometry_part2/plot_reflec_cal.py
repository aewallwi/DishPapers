...
for filename in sys.argv[1:]:
    ...
    d_cal = 10**(db_cal/20) * np.exp(2j*np.pi*ph_cal/360) # 20 to put into voltage amplitude, not power 
    

    valid = np.where(np.logical_and(fq < .200, fq > .100)) # restrict to PAPER band
    valid1 = np.where(np.logical_and(fq_cal < .200, fq_cal > .100)) # restrict to PAPER band
    
    fq, d, db, ph, fq_f,d_f, db_f, ph_f = fq[valid], d[valid], db[valid], ph[valid],fq_f[valid], d_f[valid], db_f[valid],ph_f[valid]
    fq_cal,db_cal,d_cal,ph_cal = fq_cal[valid1], d_cal[valid1], db_cal[valid1], ph_cal[valid1]
    ...
    window2 = a.dsp.gen_window(fq_cal.size, WINDOW)
