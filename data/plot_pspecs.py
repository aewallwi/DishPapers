...
#50names = ['pspecs_achrmbeam_sbinfo.npz', 'pspecs_chrmbeam_sbinfo.npz', 'pspecs_funcbeam_sbinfo.npz']
names = ['pspecs_achrmbeam_sbinfo.npz', 'pspecs_achrmbeam_sbinfo.npz_50.npz']
...
for b,bl in enumerate(files[0]['bls']):
    ...
    for k,f in enumerate(files):
        print f
        p.subplot(111)
        p.semilogy(f['hetas'][k],n.abs(f['pCnorm'][sband,b,:,12]), colors[k], label=names[k])
        p.semilogy(f['wetas'][k],n.abs(f['pInorm'][sband,b,:,12]),'--'+colors[k], label=names[k])
